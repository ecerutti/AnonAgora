...
- **Formalizar el rol del operador.** Documento de diseño descriptivo `design/modelo_operativo.md` (nombre elegido; no es ADR). Estructura y clasificación "decidido vs abierto" ya acordadas. Resultado de la clasificación: la distinción operador de capa / operador de aplicación destino está cubierta por P-0021, P-0020 y P-0006 (la arquitectura soporta que sean la misma entidad o distintas); la multiplicidad de operadores dentro de un mismo componente y la distinción hostea/administra no son decisiones de diseño del sistema sino topología de despliegue, y se describen como tales sin inventar roles formales. La separación entre operador de capa y de aplicación destino varía por despliegue (de componentes en un mismo VPS o contenedores a infraestructura separada por capa cuando el sistema crece); el documento la redacta con un hilo narrativo que deja la puerta abierta a esas variantes y a otras. Redacción pausada: depende del ADR de ausencia de perfil administrativo (ver bullet siguiente); se retoma cuando ese ADR esté cerrado.

- **Formalizar la no existencia de perfil administrativo — vía ADR.** No existirá un usuario o perfil administrativo: toda configuración del operador se hace por archivos de configuración o herramientas de línea de comando que alteran la base de datos. Esto reduce la superficie de ataque (el atacante necesita una shell del sistema o acceso de lectura-escritura sobre las tablas críticas, no solo credenciales) y cubre también las operaciones excepcionales de P-0023 (retiro de propuestas, configuración del catálogo de causales). Evaluado durante el trabajo de `modelo_operativo.md`: la decisión cumple los cuatro criterios de ADR (alternativas reales, cuestionable a futuro, impacto arquitectónico, trade-offs) y no puede escribirse sin una sección "Opciones consideradas", así que corresponde un ADR transversal propio (`design/adr/`, P-XXXX), no un párrafo descriptivo. Pendiente: crear ese ADR en una conversación dedicada. Una vez cerrado, `modelo_operativo.md` lo describe y referencia, y la "autenticación del operador" que P-0023 difirió a implementación queda resuelta por él.
